...
class GCFCalculationScene(ManimVoiceoverBase):
    ...
    def intro_scene(self):
        """Scene 1: Introduction to the main topic."""
        title = self.create_title("Greatest Common Factor (GCF)") #title describing the topic
        subtitle = Text("The largest number that divides two numbers evenly.", font_size=32) #subtitle describing the what topic is about
    ...
    def listing_factors_scene(self):
        ...
        title = self.create_title("Listing Factors") #title describing the topic
        self.ensure_group_visible(title, margin=0.5)
