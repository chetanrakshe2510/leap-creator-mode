...
1. Example Manim scenes (templates/examples/) used for one-shot or multi-shot learning
...
def get_api_doc(name: str) -> str:
    ...
    path = API_DOCS_DIR / f"{name}.md"
    try:
        return path.read_text()
    except FileNotFoundError:
        raise ValueError(f"API doc {name}.md not found in {API_DOCS_DIR}")